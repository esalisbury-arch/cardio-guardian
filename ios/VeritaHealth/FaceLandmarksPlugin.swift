// VisionCamera Frame Processor Plugin: detects a face in one camera frame
// and returns a flat record of key landmark points (eye centers/lids, mouth
// corners) for src/signal/faceSymmetryAnalysis.ts to compare left vs right.
// Uses Apple's Vision framework — no extra dependency or CocoaPod needed,
// it ships with iOS.
//
...
//
// Registered for JS as "detectFaceLandmarks" via the VISION_EXPORT_SWIFT_FRAME_PROCESSOR
// macro in FaceLandmarksPluginRegistration.m — see that file for the registration
// call. Ported from ios_native_reference/FaceLandmarksPlugin.swift.
