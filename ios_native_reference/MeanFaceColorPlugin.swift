...
// checks. See FaceLandmarksPlugin.swift if you want real face detection
// here instead of a second Vision pass per frame.
//
// This is a starting point, not a drop-in file — register it per VisionCamera's
// "Creating Frame Processor Plugins" guide (add to your Xcode target,
// register in AppDelegate or a dedicated setup file, as shown at the
// bottom). Assumes BGRA_8888 frames (VisionCamera's default iOS pixel
// format).
...
//
//   FrameProcessorPluginRegistry.addFrameProcessorPlugin("meanFaceColor") { proxy, options in
//     MeanFaceColorPlugin(proxy: proxy, options: options)
//   }
